...
CREATE POLICY "Allow users to manage their own shared links" ON "shared_links" AS PERMISSIVE FOR ALL TO "authenticated" USING (auth.uid() = "shared_links"."profile_id") WITH CHECK (auth.uid() = "shared_links"."profile_id");--> statement-breakpoint
CREATE POLICY "Allow users to manage their own stats cache" ON "stats_cache" AS PERMISSIVE FOR ALL TO "authenticated" USING (auth.uid() = "stats_cache"."profile_id") WITH CHECK (auth.uid() = "stats_cache"."profile_id");
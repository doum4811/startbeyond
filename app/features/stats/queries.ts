...
import { type StatsCacheInsert, type SharedLinkInsert, type SharedLink } from "./schema";
...
export const getSharedLinkByToken = async (
  ...
  if (error) {
    console.error("Error fetching shared link by token:", error);
...
export const upsertSharedLink = async (
  ...
  const { data: existingLink } = await client
    .from("shared_links")
    .select("id, token")
    .eq("profile_id", sharedLinkData.profile_id)
    .eq("page_type", sharedLinkData.page_type)
    .eq("period", sharedLinkData.period)
    .maybeSingle();
  ...
  if (existingLink) {
    ...
    if (updateError) {
      console.error("Error updating shared link:", updateError);
      throw updateError;
    }
    ...
    if (insertError) {
      console.error("Error inserting shared link:", insertError);
      throw insertError;
    }
